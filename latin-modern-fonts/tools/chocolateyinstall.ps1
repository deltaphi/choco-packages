...
$url        = 'http://www.gust.org.pl/projects/e-foundry/latin-modern/download/lm2.004otf.zip' # download url, HTTPS preferred
...
$packageArgs = @{
  ...
  softwareName  = 'latin-modern-fonts*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
...
#Get-ChocolateyWebFile -PackageName "latin-modern-fonts" -FileFullPath "$toolsDir/lm2.004otf.zip" -Url $url -Checksum $checksum
#Get-ChocolateyUnzip -FileFullPath "$toolsDir/lm2.004otf.zip" -Destination "$toolsDir" 
